        ...
        if depth<=max_depth:
            ...
            for nn in graph.neighbors(n):
                if nn not in seen :
                    q.append(nn)
                    #print ('neighbor: ' + nn)
                    if nn not in neighbor:
                        neighbor.add(nn)
                        node2distances[nn] = node2distances[n] + 1
                        node2num_paths[nn] = 1
                        node2parents[nn].append(n)
                        
                        #print ('adding' + nn + str(node2distances[nn]))
                        #print ('if child ' + nn + ' parent '+ n)
                    elif (node2distances[nn] == depth and n not in node2parents[nn]):
                        node2num_paths[nn] += 1
                        node2parents[nn].append(n)
                        #print ('elif child ' + nn + ' parent '+ n)
                   
    
    for node in graph.nodes():
        if node2distances[node] == 0:
            del node2distances[node]
            node2distances[root] = 0
        if node2num_paths[node] == 0:
            del node2num_paths[node]
            node2num_paths[root] = 1
        if not node2parents[node]:
            del node2parents[node]
        
    #print (node2distances.items())
...
def bottom_up(root, node2distances, node2num_paths, node2parents):
    ...
    #creating empty list in dictionary for all nodes
    ...
    #all node which are not in parent are updated to score 1
    for n in node: 
        score[n] = 1
        if n not in parent:
            score[n] = 1
    
    #scores for all the nodes
    for p in child:
        for c in child[p]:
            if p!= root:
                score[p] += score[c]/node2num_paths[c]
    
    #scores for all edges
    for p in child:
        for c in child[p]:
            result[tuple(sorted([p,c]))] = score[c]/node2num_paths[c]
...
def approximate_betweenness(graph, max_depth):
    ...
    for node in graph.nodes():
        ...
        bu = bottom_up(node, node2distances, node2num_paths, node2parent)
        for edge, score in bu.items():
            try:
                sum_of_all[edge] += score
            except KeyError:
                sum_of_all[edge] = score
    for s in sum_of_all:
        sum_of_all[s] = sum_of_all[s]/2
...
def partition_girvan_newman(graph, max_depth):
    ...
    ###TODO
    g = graph.copy()
    sorted_bness = sorted(approximate_betweenness(g, max_depth).items(), key = lambda v:v[1], reverse = True)
    components = (get_components(g))
    #print (sorted_bness)
    
    #print (sorted_bness[0][0])
    
    while len(components) == 1:
        edge_to_remove = sorted_bness[0][0]
        sorted_bness.pop(0)
...
def norm_cut(S, T, graph):
    ...
    """
    ###TODO
    cut_set = float (cut(S, T, graph))
    ncv = ((cut_set/volume(S, graph)) + (cut_set/volume(T, graph)))
    return ncv
...
def brute_force_norm_cut(graph, max_size):
    ...
    for i in range(1, max_size+1):
        for fc in combinations(graph.edges(), i):
            g.remove_edges_from(fc)

            new_components = get_components(g)     

            if len(new_components) == 2:
                ncv = norm_cut(new_components[0], new_components[1], graph)
                loE.append((ncv,list(fc)))
...
def score_max_depths(graph, max_depths):
    ...
    for md in max_depths:
        components = partition_girvan_newman(graph, md)
        ncv = norm_cut(components[0].nodes(), components[1].nodes(), graph)
        md_scores.append((md, ncv))
    return md_scores
...
def make_training_graph(graph, test_node, n):
    ...
    for i in range(n):
        remove_edges = (test_node,neighbors[i])
        g.remove_edge(*remove_edges)
    #print (g.edges())
...
def jaccard(graph, node, k):
    ...
    for n in graph.nodes():
        if (n, node) not in edges and n != node:
            neighbors2 = set(graph.neighbors(n))
            j_score = len(neighbors1&neighbors2)/len(neighbors1 | neighbors2)
            scores.append(((node,n), j_score))
