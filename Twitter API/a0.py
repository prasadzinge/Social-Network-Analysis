...
def read_screen_names(filename):
    ...
    candidate_screennames  = [i.strip()  for i in my_file.readlines()]
    my_file.close
    return candidate_screennames
...
def get_friends(twitter, screen_name):
    ...
    """
    ###TODO
    friend_IDs = robust_request(twitter, "friends/ids", {'screen_name':screen_name})
    friend_list = [friend for friend in friend_IDs]
    return friend_list
...
def friend_overlap(users):
    ...
    for user1 in users:
        for user2  in users:
            if user1!=user2:
                common_friends = len(set(user1['friends']).intersection(set(user2['friends'])))
                overlap_list.append((user1['screen_name'], user2['screen_name'], common_friends))
    """
    
    for i in range(len(users)):
        for j in range(i+1, len(users)):
            common_friends = len(set(users[i]['friends']).intersection(set(users[j]['friends'])))
            overlap_list.append((users[i]['screen_name'], users[j]['screen_name'], common_friends))
    
    
    overlap_list.sort(key = lambda c:c[2], reverse = True)
    print (overlap_list)
    return overlap_list
...
def followed_by_hillary_and_donald(users, twitter):
    ...
    for user in users:
        if user['screen_name'] == 'HillaryClinton':
            HC_Friends = set(user['friends'])
        elif user['screen_name'] == 'realDonaldTrump':
            DJT_Friends = set(user['friends'])
    
    HC_DJT_Common_IDs = robust_request(twitter, "users/lookup", {'user_id':[HC_Friends.intersection(DJT_Friends)]})
    HC_DJT_Common_friends = [screenname['screen_name'] for screenname in HC_DJT_Common_IDs ]
    return HC_DJT_Common_friends
...
def create_graph(users, friend_counts):
    ...
    nxgraph = nx.Graph()
    for user in users:
        nxgraph.add_node(user['screen_name'])
        for friend in user['friends']:
            if friend_counts[friend]>1:
                nxgraph.add_node(str(friend))
                nxgraph.add_edge(user['screen_name'],str(friend))
    return nxgraph    
...
def draw_network(graph, users, filename):
    ...
    """
    ###TODO
    user_list = [user['screen_name'] for user in users]
    nodelbl = {lbl: lbl for lbl in user_list}
    nx.draw_networkx(graph, labels = nodelbl, node_size =  70, edge_color = 'green', alpha = 0.35)
